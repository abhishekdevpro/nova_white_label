...
//       setTimeout(() => navigate(`/user/jobs-profile`), 1000);
//     }
//   }, [timeLeft]);
...
export default function VoiceInterview({ formData, onBack }) {
  ...
  useEffect(() => {
    ...
    recognition.onstart = () => {
      ...
      // Interrupt AI immediately when user starts
      if (aiSpeaking) {
        window.speechSynthesis.cancel();
        setAiSpeaking(false);
      }
  ...
  function speak(text) {
    ...
    utterance.onend = () => {
      setAiSpeaking(false);
      if (!manualStop.current && recognitionRef.current && !isListening.current) {
        recognitionRef.current.start();
        isListening.current = true;
      }
    };
  ...
  const stopSpeaking = () => {
    manualStop.current = true;
    if (isListening.current) recognitionRef.current?.stop();
    window.speechSynthesis.cancel();
    setAiSpeaking(false);
    setUserSpeaking(false);
    toast.success("🎉 Your interview is completed!");
    setTimeout(() => navigate("/user/jobs-profile"), 2000);
  };
    ...
    <div className="container py-5 text-center">
      ...
      <div className="row justify-content-center g-5 mb-4">
        <div className="col-6 col-md-3">
          <div
            className={`rounded-circle border border-4 shadow p-4 ${
              aiSpeaking ? "border-primary bg-light" : "border-secondary bg-white"
            }`}
          >
            <Bot size={64} className={aiSpeaking ? "text-primary" : "text-secondary"} />
            <p className={`mt-2 fw-semibold ${aiSpeaking ? "text-primary" : "text-muted"}`}>AI</p>
          </div>
        </div>
        ...
        <div className="col-6 col-md-3">
          <div
            className={`rounded-circle border border-4 shadow p-4 ${
              userSpeaking ? "border-success bg-light" : "border-secondary bg-white"
            }`}
          >
            <User size={64} className={userSpeaking ? "text-success" : "text-secondary"} />
            <p className={`mt-2 fw-semibold ${userSpeaking ? "text-success" : "text-muted"}`}>You</p>
          </div>
        </div>
      </div>
      ...
      {/* Buttons */}
      <div className="d-flex justify-content-center gap-3">
        <button onClick={startSpeaking} className="btn btn-success px-4">Start</button>
        <button onClick={stopSpeaking} className="btn btn-danger px-4">Stop</button>
      </div>
    </div>
  );
}
...
//         v.name.toLowerCase().includes("female")
//       );
//       setSelectedVoice(female || availableVoices[0]); // Fallback to first voice if no female
//     }
//     window.speechSynthesis.onvoiceschanged = loadVoices;
//     loadVoices();
//   }, []);
...
//           recognition.start();
//         } catch (error) {
//           console.error("⚠️ Failed to restart recognition:", error);
//         }
//       }
//     };
...
//   // Speak AI Text (interruptible, without stopping recognition)
//   function speak(text) {
//     // Do NOT stop recognition here – allow continuous listening for interruptions
...
//     utterance.onerror = (e) => {
//       console.error("⚠️ Synthesis error:", e);
//       setAiSpeaking(false);
//     };
...
//     manualStop.current = true;
//     if (recognitionRef.current) {
//       recognitionRef.current.abort(); // Use abort to fully stop
//     }
//     window.speechSynthesis.cancel();
//     setAiSpeaking(false);
//     setUserSpeaking(false);
