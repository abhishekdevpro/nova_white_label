...
//   // );
//  const employeerDetail = useSelector((state) => state.employerAuth.employerDetail);
//   // console.log(employerDetail,"employerDetailemployerDetailemployerDetail")
//   let companyDetail = {};
//   // let employeerDetail = {};
...
function EmployeeCompanyProfile() {
  ...
  // 🔹 Fetch location data
  useEffect(() => {
    getCountries(token).then((res) => setCountries(res.data.data));
  }, [token]);

  useEffect(() => {
    if (selectedCountry) {
      getStates(token, selectedCountry).then((res) => setStates(res.data.data));
    }
  }, [selectedCountry, token]);

  useEffect(() => {
    if (selectedStates) {
      getCities(token, selectedStates).then((res) => setCities(res.data.data));
    }
  ...
  const handlePhoneNumberChange = (e) => {
    const value = e.target.value;
    if (!/^\d*$/.test(value)) return showToastError("Only numbers allowed");
    if (value.length > 10) return showToastError("Max 10 digits");
    setNumber(value);
  };
                ...
                <div className="col-xl-9 col-lg-9 m-b30">
                  <div className="job-bx submit-resume">
                    ...
                    <form onSubmit={handleSubmit}>
                      <div
                        className="row m-b30"
                        // style={{ maxHeight: "calc(100vh)", overflowY: "auto", overflowX: "hidden", scrollbarWidth: "thin" }}
                      >
                        {/* Company Name */}
